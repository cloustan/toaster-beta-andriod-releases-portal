...
function normalizeCognitoGroups(raw) {
  if (Array.isArray(raw)) return raw.map(String);
  if (typeof raw === 'string' && raw.trim()) return [raw.trim()];
  return [];
}

function extractCognitoGroups(idPayload, accessPayload) {
  const merged = normalizeCognitoGroups(idPayload && idPayload['cognito:groups'])
    .concat(normalizeCognitoGroups(accessPayload && accessPayload['cognito:groups']));
  return merged.filter(function(group, index) {
    return merged.indexOf(group) === index;
  });
}

function userIsAdminFromGroups(groups) {
  return normalizeCognitoGroups(groups).some(function(group) {
    var normalized = String(group || '').toLowerCase();
    return normalized === 'admins' || normalized === 'admin';
  });
}
...
class AuthManager {
  ...
  restoreUserFromCachedTokens(tokens) {
    ...
    const groups = extractCognitoGroups(idPayload, accessPayload);
    const user = {
      ...
      groups,
      isAdmin: userIsAdminFromGroups(groups),
    ...
    try {
      ...
      return new Promise((resolve) => {
        cognitoUser.getSession((err, session) => {
          ...
          const groups = extractCognitoGroups(idToken.payload, accessToken.payload);
  ...
  onAuthStateChange(callback) {
    ...
    if (this.currentUser) {
      try {
        callback({ isAuthenticated: true, user: this.currentUser });
      } catch (_) {}
    }
    return () => {

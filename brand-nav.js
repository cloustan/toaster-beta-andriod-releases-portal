...
(function(){
  ...
  function userIsAdminFromGroups(groups){
    var normalized = Array.isArray(groups) ? groups : (groups ? [groups] : []);
    for(var i = 0; i < normalized.length; i += 1){
      var name = String(normalized[i] || '').toLowerCase();
      if(name === 'admins' || name === 'admin') return true;
    }
    return false;
  }

  function isAdminFromAuthState(state){
    if(!state || !state.isAuthenticated || !state.user) return false;
    if(state.user.isAdmin) return true;
    return userIsAdminFromGroups(state.user.groups);
  }

  async function resolveIsAdmin(){
    if (window && window.__TOASTER_SKIP_AUTH_PROBES) return false;
    try{
      if(window && window.authManager){
        if(typeof window.authManager.isAdmin === 'function' && window.authManager.isAdmin()){
          return true;
        }
        if(typeof window.authManager.checkAuthState === 'function'){
          if(typeof window.authManager.ensureSdkReady === 'function'){
            await window.authManager.ensureSdkReady();
          }
          var state = await window.authManager.checkAuthState();
          return isAdminFromAuthState(state);
        }
      }
    }catch(_){ }
    return false;
  ...
  async function waitForAuthManager(maxWaitMs){
    ...
    return (window && window.authManager) ? window.authManager : null;
  }
  ...
  ready(function(){
    try{
      ...
      var navShowAdmin = false;

      function createNavMarkup(activePath, showAdmin){
        var aiActive = activePath === 'ai.html';
        var adminActive = activePath === 'admin.html';
        ...
        var adminLink = showAdmin ? (
          '<a href="admin.html" aria-label="Admin"' + (adminActive ? ' class="is-active" aria-current="page"' : '') + '>' +
      ...
      function renderNavMarkup(){
        try {
          var markup = createNavMarkup(getPathName(), navShowAdmin);
          var inner = markup
            .replace(/^<div class="app-nav-shell nav-pill" role="navigation" aria-label="Quick navigation">/, '')
            .replace(/<\/div>$/, '');
          if (isTimerPage()) {
            var timerNav = document.getElementById('global-bottom-nav');
            if (timerNav) {
              timerNav.innerHTML = inner;
              timerNav.classList.add('app-nav-shell');
            }
            return;
          }
          var dock = document.querySelector('.app-nav-dock');
          if (!dock) {
      ...
      function applyAdminNavVisibility(isAdmin){
        if (!!isAdmin === navShowAdmin) return false;
        navShowAdmin = !!isAdmin;
        renderNavMarkup();
        bindNavHaptics();
        bindNavIsolation();
        return true;
      ...
      function refreshAdminNavLink(knownState){
        if (knownState) {
          applyAdminNavVisibility(isAdminFromAuthState(knownState));
          return Promise.resolve();
        }
        return waitForAuthManager(8000).then(function(){
      ...
      function scheduleAdminNavProbe(){
        var delays = [0, 400, 1200, 3000, 6000];
        delays.forEach(function(delay){
          setTimeout(function(){ refreshAdminNavLink(); }, delay);
        });
      }
      ...
      if (!(window && window.__TOASTER_SKIP_AUTH_PROBES)) {
        ...
        scheduleAdminNavProbe();
        try{
          if(window.authManager && typeof window.authManager.onAuthStateChange === 'function'){
            window.authManager.onAuthStateChange(function(state){
              refreshAdminNavLink(state);
            });
          }
        }catch(_){ }
